...
Les scènes:
	-Gameroot: Contient le démarrage du jeu. Ne fait rien, sauf lancer les scenes importantes pour le jeu.
...
Architecture:
	...
	Il y a 3 entités qui dérive de cette classe: Grid (qui représente la grille de jeu), Player(qui représente le joueur), et Piece (qui répresente une pièce sur la grille).
	...
	Cela me permet de ne pas dépendre de l'ordre d'execution du type : A->B donne X comme résultat mais B->A me donne un état différent de l'application.
	...
	Ainsi Grid peut être joué n'importe comment en suivant les règles de game design (elle pourrait être joué par l'IA).
